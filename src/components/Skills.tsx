...
const Skills = () => {
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      icon: <Brain size={24} />,
      skills: ["TensorFlow", "Scikit-learn", "Keras", "PyTorch", "LSTM"],
    ...
    },
    {
      ...
      icon: <Mic size={24} />,
      skills: ["Audio-to-Text", "Speech Recognition", "Signal Processing"],
  ...
  return (
    <section id="skills" className={`py-24 ${isDark ? 'bg-gray-900' : 'bg-white'} transition-colors duration-300`}>
      <div className="container mx-auto px-6 max-w-7xl">
        <div ref={ref} className={`transform transition-all duration-1000 ${
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {skillCategories.map((category, index) => (
                <div key={index} className="group relative">
                  <div className={`absolute inset-0 bg-gradient-to-r ${category.color} rounded-2xl blur opacity-10 group-hover:opacity-30 transition-opacity duration-300`}></div>
                  <div className={`relative backdrop-blur-sm rounded-2xl p-6 border h-full hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 ${
                    isDark 
            ...
            {/* Specializations */}
            <div className={`backdrop-blur-sm rounded-2xl p-8 border hover:shadow-xl transition-all duration-300 ${
              isDark 
                ? 'bg-gray-800/50 border-gray-700/50' 
                : 'bg-gray-50/80 border-gray-200/50'
            }`}>
              ...
              }`}>Areas of Specialization</h3>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {specializations.map((spec, index) => (
                  <div key={index} className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-200 hover:scale-105 ${
                    isDark 
                    ...
                    <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>{spec}</span>
                  </div>
                ))}
              </div>
            </div>
